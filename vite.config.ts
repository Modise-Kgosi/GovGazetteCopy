...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // The browser reaches this dev server through the Figma Make proxy over
      // HTTPS, so the HMR client must connect back over WSS on 443 (using the
      // page's own proxied hostname) instead of the internal localhost port.
      hmr: {
        protocol: 'wss',
        clientPort: 443,
      },
